...
func BenchmarkStream(b *testing.B) {
	...
	client := http.Client{}
	eventStremUri := fmt.Sprintf("http://%s:%s/analytics/event/streaming", configApp.App.Host, configApp.App.Port)

	for i := 0; i < b.N; i++ {
		w := strings.NewReader(batch)

		req, _ := http.NewRequest(http.MethodPost, eventStremUri, w)
